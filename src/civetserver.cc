...
bool CivetHandler::handleOptions(CivetServer* server,
                                 struct mg_connection* conn, int* status_code) {
  UNUSED_PARAMETER(server);
  UNUSED_PARAMETER(conn);
  if (status_code) {
    *status_code = -1;
...
int CivetServer::webSocketDataHandler(struct mg_connection* conn, int bits,
                                      char* data, size_t data_len,
                                      void* cbdata) {
  const struct mg_request_info* request_info = mg_get_request_info(conn);
  assert(request_info != NULL);
...
const char* CivetServer::getMethod(struct mg_connection* conn) {
  const struct mg_request_info* request_info = mg_get_request_info(conn);
  assert(request_info != NULL);
      ...
      try {
        if (r == 0) {
          ...
          break;
                   ...
                   ((conobj.postData.size() + r) > MAX_PARAM_BODY_LENGTH)) {
...
bool CivetServer::getParam(const char* data, size_t data_len, const char* name,
  ...
  if (r >= 0) {
    ...
    for (;;) {
      r = mg_get_var2(data, data_len, name, &vbuf[0], vbuf.size(), occurrence);
      if (r >= 0) {
...
std::vector<struct mg_server_port> CivetServer::getListeningPortsFull() {
  ...
  for (;;) {
    int size = mg_get_server_ports(
        context, static_cast<int>(server_ports.size()), &server_ports[0]);
